...
function check_prerequisites() {
    ...
    if [ -z "${REGION}" ]; then
        echo "Missing REGION environment variable. Using asia-south1-b"
...
function setup_mysql_database() {
    if [ "$(gcloud sql instances list --filter="name:${MYSQL_INSTANCE_NAME}" --format="value(name)" | grep "${MYSQL_INSTANCE_NAME}" || echo "empty")" == "${MYSQL_INSTANCE_NAME}" ]; then
        ...
        gcloud sql databases create bhojpur --instance="${MYSQL_INSTANCE_NAME}"
    fi

    echo "Creating Bhojpur MySQL user and setting a password..."
    MYSQL_BHOJPUR_PASSWORD=$(openssl rand -base64 20)
    export MYSQL_BHOJPUR_PASSWORD
    gcloud sql users create "${MYSQL_BHOJPUR_USERNAME}" \
        --instance="${MYSQL_INSTANCE_NAME}" --password="${MYSQL_BHOJPUR_PASSWORD}"
}
...
function install() {
    ...
    if [ "$(gcloud container clusters list --filter="name=${CLUSTER_NAME}" --format="value(name)" | grep "${CLUSTER_NAME}" || echo "empty")" == "${CLUSTER_NAME}" ]; then
        ...
        gcloud container clusters \
            ...
            --create-subnetwork name="bhojpur-${CLUSTER_NAME}" \
            --metadata=disable-legacy-endpoints=true \
    ...
    if [ "$(gcloud container node-pools list --cluster="${CLUSTER_NAME}" --region="${REGION}" --filter="name=${SERVICES_POOL}" --format="value(name)" | grep "${SERVICES_POOL}" || echo "empty")" == "${SERVICES_POOL}" ]; then
        ...
        create_node_pool "${SERVICES_POOL}" "bhojpur.net/workload_meta=true,bhojpur.net/workload_apps=true"
    fi

    if [ "$(gcloud container node-pools list --cluster="${CLUSTER_NAME}" --region="${REGION}" --filter="name=${APPLICATIONS_POOL}" --format="value(name)" | grep "${APPLICATIONS_POOL}" || echo "empty")" == "${APPLICATIONS_POOL}" ]; then
        echo "Node pool with name ${APPLICATIONS_POOL} already exists in cluster ${CLUSTER_NAME}. Skip node-pool creation step.";
    else
        create_node_pool "${APPLICATIONS_POOL}" "bhojpur.net/workload_application_services=true,bhojpur.net/workload_application_regular=true,bhojpur.net/workload_application_headless=true"
    fi
...
function uninstall() {
    ...
    if [[ $REPLY =~ ^[Yy]$ ]]; then
        ...
        kubectl delete secret clouddns-dns01-solver-svc-acct gcloud-sql-token remote-storage-gcloud bhojpur-image-pull-secret
        ...
        gcloud container node-pools delete workload-applications --region "${REGION}" --cluster "${CLUSTER_NAME}" --quiet
